...
# def generate_trajectory(initial_q, final_q, num_steps=100, weight_smoothness=10.0):
#     """
#     Generate a smooth trajectory between two configurations by minimizing the change between consecutive steps.
#     """
#     n_dofs = len(initial_q)
#     alphas = np.linspace(0, 1, num_steps)
#     initial_guess = np.array([(1 - alpha) * initial_q + alpha * final_q for alpha in alphas])
...
#         end_point_cost = np.linalg.norm(traj[0] - initial_q) ** 2 + np.linalg.norm(traj[-1] - final_q) ** 2
#         return weight_smoothness * smoothness_cost + end_point_cost
